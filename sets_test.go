...
func TestEdgeCases(t *testing.T) {
	...
	a := sets.New[int]()
	b := sets.New[int]()
	assert.True(t, sets.Equal(sets.Union(a, b), sets.New[int]()))
	assert.True(t, sets.Equal(sets.Intersection(a, b), sets.New[int]()))
	assert.True(t, sets.Equal(sets.Difference(a, b), sets.New[int]()))
	assert.True(t, sets.Equal(sets.SymmetricDifference(a, b), sets.New[int]()))

	// 하나는 비어 있고 하나는 요소 가진 경우
	sets.Add(b, 1)
	union := sets.Union(a, b)
	assert.True(t, sets.Contains(union, 1))
	intersection := sets.Intersection(a, b)
	assert.Equal(t, 0, sets.Len(intersection))
	difference := sets.Difference(a, b)
	assert.Equal(t, 0, sets.Len(difference))
	symDiff := sets.SymmetricDifference(a, b)
	assert.True(t, sets.Contains(symDiff, 1))

	// 동일한 set끼리 연산
	c := sets.New[int]()
	sets.Add(c, 1)
	sets.Add(c, 2)
	union = sets.Union(c, c)
	assert.True(t, sets.Equal(c, union))
	intersection = sets.Intersection(c, c)
	assert.True(t, sets.Equal(c, intersection))
	difference = sets.Difference(c, c)
	assert.Equal(t, 0, sets.Len(difference))
	symDiff = sets.SymmetricDifference(c, c)
	assert.Equal(t, 0, sets.Len(symDiff))
	...
	assert.Equal(t, 10000, sets.Len(large))
}
